...
Smart(
    "smart-button",
    class Button extends Smart.ContentElement {
        // Button's properties.
        ...
        _clickHandler(event) {
            ...
            if (
                (that.clickMode !== "release" &&
                    that.clickMode !== "pressAndRelease") ||
                that.readonly
            ) {
        ...
        _downHandler(event) {
            ...
            if (
                (that.clickMode === "press" || that.clickMode === "pressAndRelease") &&
                !that.readonly
            ) {
...
Smart(
    "smart-repeat-button",
    class RepeatButton extends Smart.Button {
        // button's properties.
        static get properties() {
            return {
                delay: {
...
Smart(
    "smart-toggle-button",
    class ToggleButton extends Smart.Button {
        ...
        _handleTextSelection() {
            ...
            that.timer = setTimeout(
                () => that.$.removeClass("smart-unselectable"),
                500
            );
        }
        ...
        _handleTemplate(state, oldValue) {
            ...
            if (!("content" in document.createElement("template"))) {
                that.error(
                    that.localize("htmlTemplateNotSuported", {
            ...
            for (let b = 0; b < bindingsCount; b++) {
                bindingHolderElement =
                    clone.childNodes[bindingDetails[b].childNodeIndex];
                bindingStringsCount = bindingDetails.length;
                for (let i = 0; i < bindingStringsCount; i++) {
                    bindingHolderElement.innerHTML = bindingHolderElement.innerHTML.replace(
                        bindingDetails[b].bindingString,
                        value
                    );
                }
            }
